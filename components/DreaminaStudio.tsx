      ...
      {cameraTarget && (
        <div className="fixed inset-0 z-50 bg-black flex flex-col items-center justify-center">
          ...
          <div className="flex gap-4 mt-6">
            <button
              onClick={closeCamera}
              className="px-6 py-3 rounded-xl bg-zinc-700 text-white font-medium hover:bg-zinc-600 transition"
            >
            ...
            <button
              ...
              className="px-6 py-3 rounded-xl bg-white text-black font-semibold hover:bg-zinc-200 disabled:opacity-40 transition"
            >
              📷 Chụp
            </button>
          </div>
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <button
          ...
          className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-sm transition ${
            showHistory ? "bg-violet-100 text-violet-700" : "bg-zinc-100 text-zinc-600 hover:bg-zinc-200"
          }`}
        >
          <History size={16} />
          Lịch sử
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-4">
          <div className="bg-white rounded-2xl border border-zinc-200 p-4 sm:p-6">
            ...
            <div className="mb-4">
              ...
              {personImage ? (
                <div className="relative aspect-[3/4] rounded-xl overflow-hidden border border-zinc-200 bg-zinc-50">
                  ...
                  <button onClick={() => setPersonImage(null)} className="absolute top-2 right-2 bg-black/60 text-white p-1.5 rounded-full hover:bg-black/80 transition z-10" title="Xóa">
                    <Trash2 size={14} />
                  </button>
                  <button onClick={() => openCamera("person")} className="absolute top-2 left-2 bg-black/60 text-white p-1.5 rounded-full hover:bg-black/80 transition z-10" title="Chụp từ camera">
                    <Camera size={14} />
                  </button>
                </div>
              ) : (
            ...
            {/* Cloth */}
            <div className="mb-4">
              <label className="text-xs font-semibold text-zinc-500 uppercase tracking-wider mb-2 block">👕 Ảnh quần áo</label>
              {clothImage ? (
                <div className="relative aspect-square rounded-xl overflow-hidden border border-zinc-200 bg-zinc-50">
                  ...
                  <button onClick={() => setClothImage(null)} className="absolute top-2 right-2 bg-black/60 text-white p-1.5 rounded-full hover:bg-black/80 transition z-10" title="Xóa">
                    <Trash2 size={14} />
                  </button>
                  <button onClick={() => openCamera("cloth")} className="absolute top-2 left-2 bg-black/60 text-white p-1.5 rounded-full hover:bg-black/80 transition z-10" title="Chụp từ camera">
                    <Camera size={14} />
                  </button>
                </div>
              ) : (
                <div className="grid grid-cols-2 gap-2">
                  <button onClick={() => clothInputRef.current?.click()} className="aspect-square rounded-xl border-2 border-dashed border-zinc-300 bg-zinc-50 flex flex-col items-center justify-center gap-2 hover:border-violet-400 hover:bg-violet-50 transition cursor-pointer">
                    <Upload size={24} className="text-zinc-300" />
                    <span className="text-xs text-zinc-400">Upload ảnh</span>
                  </button>
                  <button onClick={() => openCamera("cloth")} className="aspect-square rounded-xl border-2 border-dashed border-zinc-300 bg-zinc-50 flex flex-col items-center justify-center gap-2 hover:border-violet-400 hover:bg-violet-50 transition cursor-pointer">
                    <Camera size={24} className="text-zinc-300" />
                    <span className="text-xs text-zinc-400">Chụp ảnh</span>
                  </button>
                </div>
            ...
            >
              {loading ? (
                <><Loader2 size={18} className="animate-spin" /> AI đang thử đồ...</>
              ) : (
                <><Wand2 size={18} /> Thử đồ ngay ✨</>
              )}
            </button>
        ...
        <div className="bg-white rounded-2xl border border-zinc-200 p-4 sm:p-6">
          <h3 className="text-sm font-semibold text-zinc-700 mb-4 flex items-center gap-2">
            <Sparkles size={16} className="text-violet-500" /> Kết quả
          </h3>
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-2">
                  <button onClick={() => setShowZoom(true)} className="p-2 rounded-lg bg-zinc-100 text-zinc-600 hover:bg-zinc-200 transition" title="Phóng to">
                    <ZoomIn size={16} />
                  </button>
                  <button onClick={handleDownload} className="p-2 rounded-lg bg-zinc-100 text-zinc-600 hover:bg-zinc-200 transition" title="Tải về">
                    <Download size={16} />
                  </button>
                  <button onClick={handleReset} className="p-2 rounded-lg bg-zinc-100 text-zinc-600 hover:bg-zinc-200 transition" title="Làm lại">
                    <RotateCcw size={16} />
                  </button>
                </div>
              </div>
      ...
      {showZoom && resultImage && (
        <div className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center p-4" onClick={() => setShowZoom(false)}>
          <div className="relative max-w-2xl w-full max-h-[90vh]">
            ...
            <button onClick={() => setShowZoom(false)} className="absolute top-2 right-2 bg-black/60 text-white p-2 rounded-full hover:bg-black/80">
              ✕
            </button>
          </div>
        </div>
